...
To clarify, I will be cleaning the messy dataset to obtain the goal datset (not the original) as per instructions.
...
# Also changed the row names to a column variable
...
#see unique data entries under "model"
unique(clean.mtcars$model)
unique(goal.mtcars$model)

clean.mtcars <- clean.mtcars %>%
  mutate(model = str_replace_all(model, "A", "a")) %>%
  mutate(model = str_replace_all(model, "aMC Javelin", "AMC Javelin")) %>%
  mutate(model = str_replace_all(model, "T0yota Corona", "Toyota Corona")) %>%
  mutate(model = str_replace_all(model, "L0tus Europa", "Lotus Europa"))
```
